...
def _in_projection_packed(
    ...
    if k is v:
        if q is k:
            # self-attention
            return torch._C._nn.linear(q, w, b).chunk(3, dim=-1)
        else:
            ...
            return (torch._C._nn.linear(q, w_q, b_q),) + torch._C._nn.linear(
                k, w_kv, b_kv
            ).chunk(2, dim=-1)
    else:
        ...
        return (
            torch._C._nn.linear(q, w_q, b_q),
            torch._C._nn.linear(k, w_k, b_k),
            torch._C._nn.linear(v, w_v, b_v),
        )
...
def _scaled_dot_product_attention(
    q, k, v, attn_mask=None, dropout_p=None, need_weights=True, robust=False
):
    ...
    if not robust:
        ...
        attn = ch.softmax(attn)
        for _ in range(4):
            attn = attn.div(ch.sum(attn, dim=-1, keepdim=True))
            attn = attn.div(ch.sum(attn, dim=-2, keepdim=True))

    if dropout_p is not None:
        attn = torch.nn.functional.dropout(input, p=dropout_p)
...
def multi_head_attention_forward(
    ...
    attn_output, attn_output_weights = _scaled_dot_product_attention(
        q, k, v, attn_mask, dropout_p, need_weights, robust=robust
    )
    attn_output = (
