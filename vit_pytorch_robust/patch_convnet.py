# https://github.com/facebookresearch/deit/blob/main/README_patchconvnet.md
...
from .utils import SqueezeExcite, DropPath, to_2tuple, trunc_normal_
...
class Conv_blocks_se(nn.Module):
    def __init__(self, dim: int, robust=False):
        super().__init__()
        self.robust = robust
        self.qkv_pos = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(
                dim, dim, groups=dim, kernel_size=3, padding=1, stride=1, bias=True
            ),
            nn.GELU(),
...
class Layer_scale_init_Block(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_block(dim, robust=robust)
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.gamma_1 = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)
...
class ConvStem(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, padding_size: Optional[int] = None
    ) -> torch.Tensor:
        B, C, H, W = x.shape
...
class PatchConvnet(nn.Module):
    def __init__(
        ...
        if not self.multiclass:
            self.head = (
                nn.Linear(int(embed_dim), num_classes)
                if num_classes > 0
                else nn.Identity()
            )
        else:
            self.head = nn.ModuleList(
                [nn.Linear(int(embed_dim), 1) for _ in range(num_classes)]
            )
